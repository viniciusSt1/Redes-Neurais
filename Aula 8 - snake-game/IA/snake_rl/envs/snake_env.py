...
snake_image = pygame.transform.scale(snake_image, (120, 120))
...
class SnakeEnv(gym.Env, ABC):
    ...
    def __init__(self, window_size=(500, 400)):
        ...
        self.wall_sprite.fill((255, 255, 200))
    ...
    def render(self, mode='human', screen=None, first=True, human=False):
        ...
        if mode == 'human':
            ...
            title_y = 10
            death_y = self.window_size[1] - 40
            button_y = self.window_size[1] - 100
            snake_image_y = self.window_size[1] - 370

            self.blit_txt('Deep Snake', (center_div_x - 20, title_y), screen)
            self.blit_txt(f'FPS: {self.fps}', (center_div_x + 50, title_y), screen)
            self.blit_txt(f'Total Reward: {self._total_reward}', (center_div_x - 30, death_y), screen)
            self.blit_txt(f'Step: {self.count_step}', (center_div_x + 70, death_y), screen)

            button = pygame.Surface((20, 20))
            button.fill((255, 255, 200))
            pressed_button = pygame.Surface((20, 20))
            pressed_button.fill((0, 255, 0))
            ...
            self.blit_txt('Controllers', (center_div_x + 10, button_y - 150), screen)
            screen.blit(buttons[0], (center_div_x - 0, button_y - 120))
            screen.blit(buttons[1], (center_div_x + 0 + 20, button_y - 100))
            screen.blit(buttons[2], (center_div_x - 0, button_y - 80))
            screen.blit(buttons[3], (center_div_x - 0 - 20, button_y - 100))

            apple_sensor = self.observation[-8:]

            self.blit_txt('Apple Sensors', (center_div_x + 10, button_y - 50), screen)
            indexes = [0, 20, -20]
            count = 0
            for i in indexes:
                for j in indexes:
                    ...
                    if apple_sensor[count] == 1:
                        ...
                        sprite = button
                    screen.blit(sprite, (center_div_x + 0 + i, button_y + j)) #local pontos maçã
                    count += 1
            screen.blit(snake_image, (center_div_x - 50, snake_image_y))
